...
class MusicBot {
    ...
    constructor(client, options = {}) {
        ...
        switch (options.language.toLowerCase()) {
            case 'en':
                this.language = require('../../config/en.json');
                break;
            default:
                this.language = require('../../config/en.json');
    ...
    async onMessage(message) {
        ...
        if (message.content.startsWith(this.prefix + 'play') || message.content.startsWith(this.prefix + 'add') || message.content.startsWith(this.prefix + 'join')) {
            ...
            if (!serverQueue) {
                message.channel.send(new MessageEmbed().setColor('PURPLE').setTimestamp().setAuthor(this.language.embeds.createdMusicPlayer.title).setDescription(this.language.embeds.createdMusicPlayer.description));
            };
        ...
        // Stop command 
        else if (message.content === this.prefix + 'stop'|| message.content === this.prefix + 'leave') {
            if (!message.member.voice.channel) {
                return this.sendErrorEmbed(this.language.messages.voiceChannelNeeded, message);
            };
            const serverQueue = this.queue.get(message.guild.id);
        ...
        // Skip command
            ...
            return message.channel.send(new MessageEmbed().setColor('PURPLE').setFooter(`${this.language.embeds.skipSong.askedBy} ` + message.author.tag, message.author.displayAvatarURL()).setDescription('⏩ `' + serverQueue.songs[0].title + `\` ${this.language.embeds.skipSong.beenSkipped}`));
        }
        // Queue command
        ...
        // Volume command
            ...
            if (!isNaN(volumeNumber) && (volumeNumber >= 0 && volumeNumber <= 100)) {
                message.channel.send(new MessageEmbed().setColor('PURPLE').setTitle(this.language.embeds.volumeEmbed.title).setDescription(`${this.language.embeds.volumeEmbed.changedFrom} \`${serverQueue.volume * 100}\` ${this.language.embeds.volumeEmbed.to} \`${volumeNumber}\``));
        ...
        // Pause command
            ...
            if (serverQueue && serverQueue.playing) {
                ...
                return message.channel.send(new MessageEmbed().setTitle(this.language.embeds.pauseEmbed.title).setColor('PURPLE').setDescription(this.language.embeds.pauseEmbed.description).setTimestamp().setFooter(`${this.language.embeds.pauseEmbed.askedBy} ` + message.author.tag, message.author.displayAvatarURL()));
            };   
        }
        // Resume command
            ...
            if (serverQueue && !serverQueue.playing) {
                ...
                return message.channel.send(new MessageEmbed().setTitle(this.language.embeds.resumeEmbed.title).setColor('PURPLE').setDescription(this.language.embeds.resumeEmbed.description).setTimestamp().setFooter(`${this.language.embeds.resumeEmbed.askedBy} ` + message.author.tag, message.author.displayAvatarURL()));
            }; 
        }
        // Remove command
    ...
    async playSong(song, message, serverQueue) {
        ...
        if (!song) {
            ...
            return await message.channel.send(new MessageEmbed().setColor('PURPLE').setTimestamp().setAuthor(this.language.embeds.playerDestroyedEmbed.author).setDescription(this.language.embeds.playerDestroyedEmbed.description));
        };
    ...
    async getSongLyrics(args, message) {
        ...
        var msg = await message.channel.send(new MessageEmbed().setTimestamp().setAuthor(this.language.embeds.lyricsEmbed.searching, 'https://www.aerobusbcn.com/sites/all/themes/aerobus/images/ajax-loader.gif').setColor('PURPLE')) || message.send(new MessageEmbed().setTimestamp().setAuthor(this.language.embeds.lyricsEmbed.searching, 'https://www.aerobusbcn.com/sites/all/themes/aerobus/images/ajax-loader.gif').setColor('PURPLE'));
        ...
        return msg.edit(new MessageEmbed().setColor('PURPLE').setTimestamp().setFooter(this.language.embeds.lyricsEmbed.askedBy + message.author.tag, message.author.displayAvatarURL()).setTitle('📜 ' + lyrics.title).setDescription('' + lyrics.lyrics + ''))
    };
